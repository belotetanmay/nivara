...
export async function POST(
  ...
  try {
    ...
    await db.van.update({
      where: { id: vanId },
      data: {
        status: VanStatus.INACTIVE,
      },
    });

    // Write audit log
    await logAdminAction(payload.userId, 'REJECT_VAN', `Van:${vanId}`, `Rejected Van listing: ${van.title}. Reason: ${reason}`);

    return NextResponse.json({
      success: true,
      message: 'Van listing rejected and set inactive successfully',
    });
  } catch (error: any) {
