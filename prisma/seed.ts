...
async function main() {
  ...
  const vanIndiranagar = await prisma.van.create({
    data: {
      ...
      title: 'Nivara Pod Bangalore - Indiranagar',
      description: 'Located in the heart of Indiranagar, this relaxation pod offers ultimate isolation. Features a premium leather zero-gravity reclining chair, customizable ambient lighting (warm amber to soft green), and immersive spatial audio.',
  ...
  const vanKoramangala = await prisma.van.create({
    data: {
      vendorId: vendorProfileApproved.id,
      title: 'Nivara Pod Bangalore - Koramangala',
      description: 'Perfect escape for tech professionals in Koramangala. A soundproof cabin designed for quick power naps and stress recovery. Tucked in a quiet lane behind the main tech hubs.',
      amenities: ['Zero-Gravity Chair', 'Soundproofing', 'Aromatherapy', 'Ambient Lighting', 'Air Conditioning'],
  ...
  const vanUnderReview = await prisma.van.create({
    data: {
      vendorId: vendorProfileApproved.id,
      title: 'Nivara Pod Mumbai - Bandra',
      description: 'New premium pod expanding our network to Mumbai. Quiet rest and recovery cabin with state-of-the-art oxygen concentration.',
